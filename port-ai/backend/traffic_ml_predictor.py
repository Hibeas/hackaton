...
def reset_model() -> None:
    """Clear cached regressor so the next load_model() reads from disk again."""
...
def load_model(*, force: bool = False) -> Any | None:
    global _model, _model_loaded
    if force:
        reset_model()
    if _model_loaded:
...
def preload_model() -> Any | None:
    """Load (or reload) the model — call on API startup."""
